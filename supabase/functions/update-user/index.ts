...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userId && role) {
      const { error: pErr } = await supabaseAdmin.from('profiles').upsert({ id: userId, role }, { onConflict: 'id' });
      if (pErr) return new Response(pErr.message, { status: 400, headers: CORS_HEADERS });
    }

    return new Response("User updated", { status: 200, headers: CORS_HEADERS });
  } catch (err) {
